...
let DATA_ROOT = path.join(__dirname, 'data');

//const LOCAL_DATA = path.join(__dirname, 'data', 'local');   // local/LAN
//const PUBLIC_DATA = path.join(__dirname, 'data', 'public'); // external
...
function getRequestOrigin(req, urlObj) {
  // force overrides via query params
  if (urlObj.searchParams.get('forceLocal') === 'true') return 'localhost';
  if (urlObj.searchParams.get('forceLAN') === 'true') return 'lan';
  if (urlObj.searchParams.get('forcePublic') === 'true') return 'external';
...
const server = http.createServer((req, res) => {
  ...
  const origin = getRequestOrigin(req, urlObj);
  console.log(urlObj,origin);

  //   let DATA_ROOT;
  if (origin === 'localhost') {
    DATA_ROOT = path.join(__dirname, 'data', 'localhost');
  } else if (origin === 'lan') {
    DATA_ROOT = path.join(__dirname, 'data', 'lan');
  } else {
    DATA_ROOT = path.join(__dirname, 'data', 'public');
  }
